...
public class SystemSettingsRepository(IConfiguration config)
{
  ...
  public async Task<IEnumerable<SystemSetting>> GetAll()
  {
    ...
    return await conn.QueryAsync<SystemSetting>(
      "SELECT [key] AS [Key], [value] AS [Value] FROM system_settings ORDER BY [key]");
  }

  public async Task<string?> GetValue(string key)
  {
    await using var conn = GetConnection();
    return await conn.ExecuteScalarAsync<string?>("SELECT [value] FROM system_settings WHERE [key] = @key",
      new { key });
  }
